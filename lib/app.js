...
class Scissor {
  ...
  dash() {
    ...
    setTimeout(() => {this.cooldown = false;console.log('ready');}, 500);
  }

  updatePostition() {
    let theta = Math.atan2((this.pointer().y - 300), (this.pointer().x - 500)) * 180 / Math.PI;
    theta = (theta + 90) % 360;
...
const update = (e) => {
  ...
  // if (enemySprite.hitTest(lHit.x, lHit.y) || enemySprite.hitTest(rHit.x, rHit.y)) { debugger;enemySprite.alpha = 0.5 }
  stage.update(e);
};

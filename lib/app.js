...
class Game {
  ...
  addGame() {
    ...
    while (Object.keys(this.players).length < 14) {
  ...
  checkCollisions() {
    for (var i = 0; i < Object.keys(this.players).length; i++) {
      for (var j = 0; j < Object.keys(this.players).length; j++) {
        ...
        if (otherId !== id) {
          if (this.swordCollide(this.players[id], this.players[otherId])) {
            this.players[otherId].handleKnockback(this.players[id], this.players[id].dashing ? 10 : 5);
          } else if (this.handCollide(this.players[id], this.players[otherId])) {
            this.arena.removeChild(this.players[otherId].sprite);
            delete this.players[otherId];
          }
        }
      }
    }

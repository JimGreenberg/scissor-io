...
export default class Scissor {
  ...
  // angle() {
  //   let theta = (this.leftBlade.angle + this.rightBlade.angle / 2) % tau;
  ...
  turnTo() {
    ...
    if (Math.floor(_.toDeg(currAng) / 9) !== Math.floor((_.toDeg(theta) + 360) % 360 / 9)) {
      let turn = (currAng - theta + tau) % tau > Math.PI ? 0.05 : -0.05;
      this.angle += turn;
  ...
  dash() {
    ...
    // this.comp.forEach(body => Matter.Body.applyForce(body, {x: 0, y: 0}, {x:0, y: -0.01}));
    // setTimeout(() => {this.cooldown = false;}, 1000);
    // this.leftBlade.torque = -0.5;
    // this.rightBlade.torque = 0.5;
    // Matter.Body.applyForce(this.leftCircle, {x: 0, y: 0}, {x:0, y: -0.01});
    game.render.options.wireframes = !game.render.options.wireframes;
    console.log(_.toDeg(this.leftBlade.angle));
    console.log(_.toDeg(this.rightBlade.angle));
    console.log(_.toDeg(this.angle));
    // Matter.Composite.rotate(this.composite, 0.2, this.pos());
  }

  updatePosition() {

    this.composite.bodies.forEach(body => {
      const mouse = Matter.Vector.create(this.pointer().x - this.x, this.pointer().y - this.y);
      const vel = Matter.Vector.mult(Matter.Vector.normalise(mouse), 2);
  ...
  getTurn() {
    ...
    return theta = (_.toDeg(theta) + 90) % 360;
  ...
  getVel() {
    this.vx = Math.sin(this.composite.angle);
    this.vy = -Math.cos(this.composite.angle);
